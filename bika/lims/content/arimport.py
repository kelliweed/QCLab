...
class ARImport(BaseFolder):
    ...
    def workflow_script_import(self):
        ...
        profiles = [x.getObject() for x in pc(portal_type='AnalysisProfile')]
    ...
    def save_sample_data(self):
        ...
        """
        pc = getToolByName(self, 'portal_catalog')
        keywords = pc.uniqueValuesFor('Keyword')
        profiles = []
    ...
    def get_row_services(self, row):
        ...
        for val in row.get('Analyses', []):
            brains = pc(portal_type='AnalysisService', Keyword=val)
            if not brains:
                brains = pc(portal_type='AnalysisService', title=val)
            if not brains:
                brains = pc(portal_type='AnalysisService', UID=val)
            if brains:
    ...
    def get_row_container(self, row):
        ...
        if val:
            brains = pc(portal_type='Container', UID=row['Container'])
            if brains:
                brains[0].getObject()
            brains = pc(portal_type='ContainerType', UID=row['Container'])
            if brains:
    ...
    def get_row_profiles(self, row):
        ...
        for profile_title in row.get('Profiles', []):
            profile = pc(portal_type='AnalysisProfile', title=profile_title)
