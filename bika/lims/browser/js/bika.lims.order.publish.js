/**
 * Controller class for Order Publish view
 */
function OrderPublishView() {
    ...
    /**
     * Entry-point method for OrderPublishView
     */
    that.load = function() {
        ...
        var cookiename = "order.publish.view.referrer";
        ...
        $('#order_publish_container a[href^="#"]').click(function(e) {
            e.preventDefault();
        ...
        $('#publish_button').click(function(e) {
            ...
            $('#order_publish_container').animate({opacity:0.4}, 'slow');
            var count = $('#order_publish_container #report .order_publish_body').length;
            $('#order_publish_container #report .order_publish_body').each(function(){
    ...
    function load_layout() {
        ...
        $('#order_publish_container').css({'width':dim.width + 'mm', 'padding': '0mm '+dim.marginRight + 'mm 0mm '+dim.marginLeft +'mm '});
        $('#order_publish_header').css('margin', '0mm -'+dim.marginRight + 'mm 0mm -' +dim.marginLeft+'mm');
        $('div.order_publish_body').css({'width': dim.width + 'mm', 'max-width': dim.width + 'mm', 'min-width': dim.width + 'mm'});

        // Iterate for each AR report and apply the dimensions, header,
        // footer, etc.
        $('div.order_publish_body').each(function(i) {
            ...
            // within div.order_publish_body are checked and will be
            // treated as nob-breakable elements. So, if a div element
            // from within a div.order_publish_body is taller than the
            ...
            $(this).children('div:visible').each(function(z) {
                ...
                // the div.order_publish_body container, not wrapped by
                // other div element.
